            ...
            <div className="bg-white rounded-3xl shadow-trust p-8 max-w-2xl mx-auto border border-trust-100">
              <div className="text-center">
                ...
                <div className="flex flex-col items-center space-y-6">
                  ...
                  <div className="relative w-80 h-80 sm:w-96 sm:h-96 cursor-pointer group transition-all duration-500 ease-out hover:scale-105" onClick={() => setIsPlaying(!isPlaying)}>
                    ...
                    <div className={`w-full h-full rounded-full relative overflow-hidden transition-all duration-700 ease-out ${isPlaying ? 'animate-pulse-gentle' : ''} group-hover:shadow-2xl group-hover:shadow-green-500/30`}>
                      ...
                      <div className={`absolute inset-0 transition-all duration-700 ${isPlaying ? 'opacity-80 animate-spin-slow' : 'opacity-60'} group-hover:opacity-75`}>
                        {[...Array(12)].map((_, i) => (
                          <div
                            key={i}
                            className="absolute w-0.5 bg-gradient-to-r from-transparent via-white to-transparent transition-all duration-300"
                            style={{
                              height: '50%',
                              left: '50%',
                              top: '0%',
                      ...
                      <div className={`absolute inset-0 transition-all duration-1000 ${isPlaying ? 'animate-reverse-spin-slow' : ''}`}>
                        {[...Array(8)].map((_, i) => (
                          <div
                            key={i}
                            className="absolute w-full h-full bg-gradient-conic from-green-700/30 via-transparent to-transparent transition-opacity duration-500"
                            style={{
                      ...
                      {isPlaying && (
                        <>
                          <div className="absolute inset-0 bg-gradient-radial from-white/30 to-transparent animate-pulse-fast"></div>
                    ...
                    <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 transition-all duration-300">
                      <div className={`bg-white rounded-full px-6 py-3 sm:px-8 sm:py-4 shadow-lg flex items-center space-x-3 transition-all duration-300 group-hover:shadow-2xl group-hover:shadow-green-500/20 ${isPlaying ? 'scale-105 shadow-xl' : 'hover:scale-110'} active:scale-95`}>
                        <div className={`w-8 h-8 sm:w-10 sm:h-10 bg-black rounded-full flex items-center justify-center transition-all duration-300 ${isPlaying ? 'animate-pulse-gentle' : ''} group-hover:bg-green-800`}>
                          <svg 
                            viewBox="0 0 24 24" 
                            className={`w-4 h-4 sm:w-5 sm:h-5 text-white transition-all duration-300 ${isPlaying ? 'scale-90' : ''}`}
                            fill="currentColor"
                          >
                            {isPlaying ? (
                        ...
                        <span className={`text-black font-medium text-lg sm:text-xl transition-all duration-300 ${isPlaying ? 'text-green-800' : ''} group-hover:text-green-800`}>
                          {isPlaying ? 'Listening...' : 'Meet Sarah'}
                        </span>
                      </div>
                    </div>
                    ...
                    {isPlaying && (
                      <>
                        ...
                        <div className="absolute inset-0 w-full h-full bg-green-300 rounded-full animate-ping opacity-15 scale-125" style={{animationDelay: '0.5s'}}></div>
                  ...
                  {isPlaying && (
                    <div className="text-center animate-fade-in-up">
                      <div className="inline-flex items-center bg-gradient-to-r from-green-50 to-emerald-50 border border-green-200 rounded-full px-6 py-3 sm:px-8 sm:py-4 shadow-soft hover:shadow-green-200/50 transition-all duration-300">
                        <div className="relative mr-3">
                          <div className="w-3 h-3 bg-green-500 rounded-full animate-pulse"></div>
                          <div className="absolute inset-0 w-3 h-3 bg-green-400 rounded-full animate-ping opacity-40"></div>
                        </div>
                        <span className="text-green-700 font-medium text-sm sm:text-base">Sarah is introducing herself...</span>
                  ...
                  {!isPlaying && (
                    <div className="text-center animate-fade-in-up">
                      <p className="text-trust-500 text-sm sm:text-base italic">
                        ✨ Experience Sarah's warm, caring voice
                      </p>
                    </div>
                  )}
                </div>
