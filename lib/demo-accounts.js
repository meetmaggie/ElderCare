...
// Helper function to check and fix missing demo data
export async function checkAndFixDemoData(supabase, userEmail, userId) {
  try {
    ...
    // If either is missing and this is a demo account, recreate the data
    if ((familyError || elderlyError) && demoAccounts[userEmail]) {
      console.log(`Recreating demo data for ${userEmail}`)
      const accountData = demoAccounts[userEmail]
      ...
      if (familyError) {
        ...
        if (familyInsertError) {
          console.error(`Error recreating family user for ${userEmail}:`, familyInsertError)
        }
      }

      // Create or update elderly user
      if (elderlyError) {
        const { error: elderlyInsertError } = await supabase
          .from('elderly_users')
          .upsert([
            {
        ...
        if (elderlyInsertError) {
          console.error(`Error recreating elderly user for ${userEmail}:`, elderlyInsertError)
        }
      }

      return true // Data was recreated
    }

    return false // Data was already complete
  } catch (error) {
    console.error(`Error checking demo data for ${userEmail}:`, error)
    return false
  }
}
...
export async function setupDemoAccounts(supabase) {
  ...
  for (const [email, accountData] of Object.entries(demoAccounts)) {
    try {
      ...
      if (!userId) {
        console.warn(`Could not get user ID after signup/signin for ${email}`)
        continue
      }
      // Insert family user
      ...
      if (elderlyError) {
        ...
        // Try without ID to let Supabase generate one
        const { data: elderlyData2, error: elderlyError2 } = await supabase
          .from('elderly_users')
          .insert({
            family_user_id: userId,
            name: accountData.elderlyData.name,
        ...
        if (elderlyError2) {
          console.error(`Error creating elderly user (second attempt) for ${email}:`, elderlyError2)
          results.push({ email, status: 'error', error: elderlyError2.message })
          continue
        }
      }
