...
export class CallingScheduler {
  ...
  async initiateCall(user) {
    try {
      ...
      const twilioResponse = await fetch(`${process.env.NEXT_PUBLIC_SITE_URL || 'http://localhost:3000'}/api/make-outbound-call`, {
        method: 'POST',
        headers: {
      ...
      if (!twilioResponse.ok) {
        ...
        throw new Error(`Twilio call failed: ${errorData.error}`)
      }
  ...
  async prepareUserContext(user) {
    // Get recent conversation data with more details
    const { data: recentCalls } = await supabase
      .from('call_records')
      .select('key_topics, summary, mood, transcript, call_date')
      .eq('elderly_user_id', user.id)
      .eq('status', 'completed')
      .order('call_date', { ascending: false })
      .limit(3)
    ...
    const hobbies = this.extractHobbies(recentCalls)
    const familyUpdates = this.extractFamilyInfo(recentCalls)
    const healthMentions = this.extractHealthMentions(recentCalls)
    ...
    const totalCalls = await this.getTotalCallCount(user.id)
    const averageMood = this.calculateAverageMood(moodHistory)
    const lastCallDate = recentCalls?.[0]?.call_date || null
    ...
    return {
      ...
      // Dynamic content from conversations
      hobbies: hobbies,
      ...
      current_mood_trend: averageMood,
      ...
      // Agent guidance
      agent_guidance: this.generateAgentGuidance(user, recentCalls, moodHistory)
    }
  }

  // Extract hobbies and interests from conversation history
  extractHobbies(calls) {
    const hobbies = new Set()
    const hobbyKeywords = [
      'garden', 'gardening', 'reading', 'books', 'cooking', 'baking',
      'knitting', 'sewing', 'painting', 'music', 'walking', 'exercise',
      'crossword', 'puzzle', 'television', 'tv', 'movies', 'crafts'
    ]
    ...
    return Array.from(hobbies).slice(0, 5)
  }

  // Extract family information from conversations
  extractFamilyInfo(calls) {
    const familyInfo = []
    const familyKeywords = ['son', 'daughter', 'grandchild', 'grandson', 'granddaughter', 'family', 'visit', 'children']
    
    calls?.forEach(call => {
      if (call.summary && call.summary.toLowerCase().includes('family')) {
        familyInfo.push({
          date: call.call_date,
          info: call.summary
        })
    ...
    return familyInfo.slice(0, 3)
  }

  // Extract health mentions from recent conversations
  extractHealthMentions(calls) {
    const healthMentions = []
    
    calls?.forEach(call => {
      if (call.key_topics?.includes('Health') || 
          (call.summary && call.summary.toLowerCase().includes('health'))) {
        healthMentions.push({
          date: call.call_date,
          mention: call.summary,
          mood: call.mood
        })
      }
    })
    
    return healthMentions.slice(0, 3)
  }

  // Get total call count for user
  async getTotalCallCount(userId) {
  ...
  calculateAverageMood(moodHistory) {
    ...
    const avgScore = moodHistory.reduce((sum, mood) => sum + (mood.mood_score || 3), 0) / moodHistory.length
    
    if (avgScore >= 4.5) return 'Very Happy'
    if (avgScore >= 3.5) return 'Content'
    if (avgScore >= 2.5) return 'Neutral'
    if (avgScore >= 1.5) return 'Concerned'
    return 'Needs Attention'
  }

  // Generate specific guidance for the agent based on user data
  generateAgentGuidance(user, recentCalls, moodHistory) {
    const guidance = []
    
    // First call guidance
    if (!user.first_call_completed) {
      guidance.push("This is a discovery call - focus on getting to know the user, their interests, family, and daily routine")
    } else {
      guidance.push("This is a follow-up call - reference previous conversations and check on topics mentioned before")
    }
    
    // Mood-based guidance
    const recentMood = moodHistory?.[0]
    if (recentMood && recentMood.mood_score <= 2) {
      guidance.push("User showed low mood in recent call - be extra gentle and supportive")
    }
    
    // Health-based guidance
    const hasHealthMentions = recentCalls?.some(call => call.key_topics?.includes('Health'))
    if (hasHealthMentions) {
      guidance.push("User has mentioned health topics recently - check in on their wellbeing")
    }
