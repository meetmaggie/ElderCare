...
export class CallingScheduler {
  ...
  async initiateCall(user) {
    try {
      console.log(`Initiating call for ${user.name} (${user.phone_number})`)
      ...
      const nextCall = this.calculateNextCall(user.call_frequency)
  ...
  async prepareUserContext(user) {
    ...
    const { data: recentCalls } = await supabase
      ...
      .eq('call_log_id.user_id', user.id)
